...
class Resource_model extends My_Model {
	...
	function getList($param = null)
	{
		$user_id = isset($param['user_id']) ? $param['user_id'] : 0;
                $personal = isset($param['personal']) ? $param['personal'] : FALSE;
                if($personal){
                    $str = "SELECT p.* FROM {$this->main_table} p  WHERE p.status = 1 AND p.uid = ? ORDER BY p.id DESC" ; 
	...
	{    
		...
		$id = $this->db->insert_id();
                ...
                if($obj['destination']){
                    ...
                    $this->db->insert_batch('resource_destination', $destinations);
                }
